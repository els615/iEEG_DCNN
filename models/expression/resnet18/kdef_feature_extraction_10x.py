# resnet18 no softmax trained to do expression recog
...
save_dir = '/mmfs1/data/schwarex/ieeg/neuralNetworks/expression/resnet_noSoftmax/kdef_features'
...
for rep in range(2,12): 
    checkpoint_file = '/mmfs1/data/schwarex/ieeg/neuralNetworks/expression/resnet_noSoftmax/199_%d.ckpt' % rep # each checkpoint file really should be num -1 
    ...
    df2.to_pickle(save_dir+'/kdef_features_%d.pkl' % (rep-1))
    # merge
    new_df2 = pd.merge(df,df2,on=['index'])
    new_df2.to_pickle(save_dir+'/kdef_features_merged_%d.pkl' % (rep-1))
