# resnet18 no softmax pretrained to do object recog 
# summing 3 channels for greyscale kdef
# extract kdef features
import pandas as pd
import torch
...
save_dir = '/mmfs1/data/schwarex/ieeg/neuralNetworks/object/resnet_noSoftmax/kdef_features'
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, # change 100 to 7
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, inchans=1):
...
def _resnet(arch, block, layers, pretrained, inchans=1, **kwargs):
    model = ResNet(block, layers, **kwargs)
    if pretrained:
        savedModel=torch.load('/mmfs1/data/schwarex/ieeg/neuralNetworks/object/resnet_noSoftmax/resnet18-5c106cde_2.pth')
        #savedModel=torch.load('/Users/emilyschwartz/Downloads/resnet18-5c106cde_2.pth')
        state_dict=savedModel
...
def _load_pretrained(model, url, inchans=3):
    ...
    model.load_state_dict(state_dict)
'''
def resnet18(pretrained=False, inchans=1, **kwargs):
    ...
    if pretrained:
        model = _resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, inchans, **kwargs)
    return model
...
class KDEFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.dataAll.iloc[idx,4] #exp labels are not coded same way as fer2013
...
net = resnet18(pretrained=True, inchans=1)
net.to(device)
...
df2.to_pickle(save_dir+'/kdef_features.pkl')
# merge
new_df2 = pd.merge(df,df2,on=['index'])
new_df2.to_pickle(save_dir+'/kdef_features_merged.pkl')
