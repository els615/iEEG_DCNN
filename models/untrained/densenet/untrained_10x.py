...
save_dir = '/mmfs1/data/schwarex/ieeg/neuralNetworks/untrained'
...
import pandas as pd
# df = pd.read_pickle('/mmfs1/data/schwarex/neuralNetworks/datasets/KDEF_3views.pkl')
...
class KDEFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.dataAll.iloc[idx,0] # 0 identity labels, 3
...
for rep in range(1,11):
    ...
    with torch.no_grad():
        for i, data in enumerate(testloader):
            ...
            #tmp = torch.squeeze(labels.type(torch.LongTensor))
            tmpInd = []
            #tmpInd = torch.squeeze(indices.type(torch.LongTensor))
    ...
    list_of_tuples = list(zip(image_pred_final, image_pred_layer1, image_pred_layer2, image_pred_layer3, image_pred_layer4, idx_arr)) 
    df2 = pd.DataFrame(list_of_tuples, columns = ['final_output', 'conv1', 'lay1', 'lay2', 'lay3', 'index'])
    df2.to_pickle(save_dir+'/kdef_features_%d.pkl' % rep) 
    # merge
    new_df2 = pd.merge(df,df2,on=['index'])
    new_df2.to_pickle(save_dir+'/kdef_features_untrain_merged_%d.pkl' % rep)
